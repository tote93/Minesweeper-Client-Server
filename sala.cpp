			...
			for (int i = fila - 1; i < fila + 2; i++)
			{
				for (int j = col - 1; j < col + 2; j++)
				{
					if (i >= 0 && i < 10 && j >= 0 && j < 10)
					{
						...
						{
							if (_mPublica[i][j] == "A" || _mPublica[i][j] == "B" || _mPublica[i][j] == "AB")
							{
								_mPOculta[i][j] = "J";
								buscarMinas(i, j);
							}
...
{
	return (_mPublica[fila][col].compare("-") == 0 || _mPublica[fila][col].compare("A") == 0
	        || _mPublica[fila][col].compare("B") == 0) ? true : false;
}